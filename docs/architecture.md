...
Generator4 resolves a campaign slot from a baked rules dataset (`src/ualg/data/gen4_rules.json`), keeps the archetype tileset and dimensions, synthesizes new WFC `typ_map` terrain, and rebuilds ownership, height, buildings, hosts, squads, gates, items, upgrade sectors, and authored-style power/flak/radar infrastructure while constraining enemy rosters and tech progression to the source slot. Rules v2 records include terrain profiles and infrastructure placements extracted from the original maps, so installed packages can generate varied height fields and strategic station layers without `original-levels/`. Its modules live under `ualg.gen4`; `tools/build_gen4_rules.py` refreshes the baked rules from `original-levels/`.
