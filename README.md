...
- `generator4`: Campaign-aware hybrid synthesis. It generates new WFC terrain like Generator3 synthesis, but constrains each level by its campaign slot: exact archetype size/tileset, source-level enemy enables, upgrade sectors, startup tech, gate targets, authored-style height profiles, and pre-placed power/flak/radar infrastructure.
...
- `generator4` reads `src/ualg/data/gen4_rules.json`, generated by `tools/build_gen4_rules.py` from the same raw original levels. The v2 rules include terrain profiles and strategic infrastructure placements; installed packages do not need `original-levels/`.
